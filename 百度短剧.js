...
function init(cfg) {
    siteName = cfg.skey?.split('_')[1] || cfg.skey || '百度短劇';
    siteKey = cfg.skey;
    siteType = cfg.stype;
}
...
async function homeVod() {
    ...
    const list =
        safeJSONParse(categoryResult).list || [];
...
async function category(tid, pg, filter, extend) {
    ...
    /*
      ...
      原版使用 CryptoJS MD5。
      目前先固定使用 v2，
      用來確認 WebHTV 首頁空白是否與 CryptoJS 有關。
    */
    let version = "v2";
...
async function detail(id) {
    const res = await request(
        `${rule.detailHost}${rule.detailUrl}`,
        {
            method: 'POST',
            data: {
                playlet_id: id,
    ...
    const vod = {
        vod_id: id,
        vod_name:
            dthtml.playlet_title ||
            '未知剧名',

        vod_pic:
            dthtml.playlet_poster ||
            '',
        ...
        vod_remarks:
            `共${vids.length}集 热度值:${dthtml.hot_value || 0}`,
...
async function play(flag, id, flags) {
    ...
    const urls = video.clarityUrl
        .filter(item => item?.url)
        .map(item => ({
            title: item.title,
            url: item.url,
            order:
                clarity_order[item.title] ||
                999
        }))
        .sort(
            (a, b) =>
